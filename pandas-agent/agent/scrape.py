...
def scrape_pandas_website():
    ...
    for parent_functions in req_l1_elems:
        for func in parent_functions.find_all('a'):
            ...
            if "#" not in href and "api/pandas" not in href:
                first_level.update({href:{"functions":[],"name":func.text,"url":base_func_url+href}})
                curr_parent = href
            else:
    ...
    with open(config_params['FUNCTION_CALLING_DATASET']['FUNCTION_SAVE_DEST'], "w", encoding="utf-8") as json_file:
        json_file.write(json_data)

    print("Data has been successfully written to", config_params['FUNCTION_CALLING_DATASET']['FUNCTION_SAVE_DEST'])
