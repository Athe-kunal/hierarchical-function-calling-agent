...
def build_no_summary_graph():
    with open(config_params['FUNCTION_CALLING_DATASET']['FUNCTION_SAVE_DEST'], "r", encoding="utf-8") as json_file:
        # Load the JSON data from the file
        data = json.load(json_file)
    pandas_graph = nx.DiGraph()

    parent_names = [(data[d]['name'],{"url":data[d]['url'],"type":"parent_node"}) for d in data]
...
def build_graph():
    with open(config_params['FUNCTION_CALLING_DATASET']['FUNCTION_SAVE_DEST'], "r", encoding="utf-8") as json_file:
        # Load the JSON data from the file
        data = json.load(json_file)
    with open(config_params['PARENTS_SUMMARY']['SUMMARY_JSON_FILE_PATH'], "r", encoding="utf-8") as json_file:
        # Load the JSON data from the file
